...
SQL. Siempre devolverá un conjunto de documentos, que pueden variar desde no devolver
ningún documento, a devolver todos los de la colección.
...
La función **find()** puede tener varios parámetros.
  ...
  * El primero indica una condición o criterio, y devolverá aquellos documentos de la colección que cumplan la condición o criterio. Esta condición viene dada en forma de documento (u objeto) JSON, y es como la habíamos visto en la función **update()** :
      ...
      db.collection.find( { clave1 : valor1 } )

Devolverá todos los documentos de la colección **collection** que tengan el
campo **clave1** y que en él tengan el valor **valor1**.

Este criterio puede ser tan complicado como haga falta, formándolo en JSON. Puede
...
en caso de que tenga más de un miembro en la condición que devolvería aquellos documentos de la **collection** que tienen el campo
**clave1** con el valor **valor1** y que tienen el campo **clave2** con el valor
**valor2**

    db.collection.find( { clave1 : valor1 , clave2 : valor2 } )



Si no queremos poner ningún criterio, para que los devuelva todos, no ponemos nada como parámetro, o aún mejor, le pasamos un documento (objeto) vacío, de forma
...
Si ponemos algún campo a que sí que aparezca (es decir, con el valor 1), los únicos
...
Y si queremos mostrar únicamente el nombre:
...
o bonita (_pretty_), pondremos esta función al final: **find().pretty()**
